...
int main()
{
	...
	bool bForceOpll = true/*!gpio_get(MGSPICO_SW2)*/;
	bool bMuSICA = !gpio_get(MGSPICO_SW1);
	bool bForceTGF = gpio_get(MMM_MODESW);		// TGF or MGS_KIN5
	busy_wait_ms(100);
	bForceOpll &= true/*!gpio_get(MGSPICO_SW2)*/;
	bMuSICA &= !gpio_get(MGSPICO_SW1);
	bForceTGF &= gpio_get(MMM_MODESW);
	if( bForceTGF || sd_fatExistFile("conf\\TGFMODE") ) {
		g_MusType = MUSICTYPE_TGF;
		bForceOpll = true;
	}
	else if( bMuSICA || sd_fatExistFile("conf\\KIN5MODE") ) {
		g_MusType = MUSICTYPE_KIN5;
		bMuSICA = true;
	}
	else {
		g_MusType = MUSICTYPE_MGS;
	}
	...
	oled.ResetI2C();
	oled.Clear();
	oled.Strings8x16(1*8+4, 0*16, "MGS MUSE", false);
	oled.Strings8x16(1*8+4, 1*16, "MACHINA v1.6", false);
	oled.Box(6, 0, 104, 30, true);
...
#ifdef MGS_MUSE_MACHINA
	gpio_put(MMM_S_RESET, 1);		// RESET信号を解除
#else
